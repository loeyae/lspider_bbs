...
class RepliesUniqueDB(Mongo, BaseRepliesUniqueDB, SplitTableMixin):
    ...
    __tablename__ = 'replies_unique'

    def __init__(self, connector, table=None, **kwargs):
        super(RepliesUniqueDB, self).__init__(connector, table = table, **kwargs)
    ...
    def insert(self, obj, ctime):
        ...
        try:
            _id = super(RepliesUniqueDB, self).insert({"unid": unid, "ctime": ctime}, table=table)
            if _id:
    ...
    def _table_name(self, unid):
        return super(RepliesUniqueDB, self)._collection_name(unid[0:1])
