...
class BbsItemHandler(BaseHandler):
    ...
    def run_parse(self, rule):
        ...
        parsed = {}
        if self.page == 1:
            main_parser = ItemParser(source=self.response['content'], ruleset=copy.deepcopy(rule['one']), log_level=self.log_level, url=self.response['final_url'])
    ...
    def _build_crawl_info(self, final_url):
        ...
        """
        self.task['article']['crawlinfo']['mode'] = self.mode
        if 'final_url' not  in self.task['article']['crawlinfo']:
            self.task['article']['crawlinfo']['final_url'] = {str(self.page): final_url}
        else:
            self.task['article']['crawlinfo']['final_url'][str(self.page)] = final_url
        if 'forumRule' not  in self.task['article']['crawlinfo']:
